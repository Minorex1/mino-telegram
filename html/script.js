...
$(".closePostoffice").on("click",function(){
    $.post('http://mino-telegram/NUIFocusOff', JSON.stringify({}));
});
//Form 
$("#composeForm").submit(function(e){
    ...
    $.post('http://mino-telegram/sendTelegram', JSON.stringify({recipient:recipient,subject:subject,message:message,postoffice:postname}));
    //Reset form 
    $("#recipient").val('');
    $("#subject").val('');
    $("#message").val('');
...
$(document).ready(function(){
    $("#inboxList").on("click",'li',function(event){
        ...
        $.post('http://mino-telegram/getview', JSON.stringify({id: $(this).data('id')}));
       $(".inbox").fadeOut().hide();
       $(".view").fadeIn().show();
    });
});
...
$(".telegram_delete_button").click(function(event){
    $.post('http://mino-telegram/delete', JSON.stringify({id: itemToDel}));
    $(".inbox").fadeIn().show();
    $(".view").fadeOut().hide();
});
